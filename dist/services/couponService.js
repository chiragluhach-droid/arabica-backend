 ...
 */
async function validateCoupon({ code, userId, branchId, subtotal, itemProductIds, }) {
    const coupon = await Coupon_1.Coupon.findOne({ code: code.toUpperCase(), isActive: true });
    if (!coupon)
        return { valid: false, reason: "Invalid coupon code", discount: 0 };
    const now = new Date();
    ...
    if (coupon.type === "FirstOrder") {
        const prior = await Order_1.Order.countDocuments({ user: userId });
        if (prior > 0)
            return { valid: false, reason: "Valid on your first order only", discount: 0 };
    }
...
async function consumeCoupon(couponId, userId) {
    await Coupon_1.Coupon.findByIdAndUpdate(couponId, {
        $inc: { usedCount: 1 },
        $push: { usedBy: userId },
    });
}
